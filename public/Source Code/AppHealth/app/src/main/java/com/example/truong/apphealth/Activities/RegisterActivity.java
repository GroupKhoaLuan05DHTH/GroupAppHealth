        ...
        call.enqueue(new Callback<Check>() {
            ...
            public void onResponse(Response<Check> response, Retrofit retrofit) {
                if (response.isSuccess()) {
                    ...
                    if (check) {
                        Toast.makeText(RegisterActivity.this, "Cập Nhập frofile thành công", Toast.LENGTH_SHORT).show();
                        if (Instance.profile != null) {
                            apigetProfile(Instance.profile.get(0).ID);
                        } else {
                        }
                    } else {
                        Toast.makeText(RegisterActivity.this, "Cập nhập thất bại", Toast.LENGTH_SHORT).show();
                    }
